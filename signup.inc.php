...
header("Location: ../includes/signup.php?error=emptydfields&uname");
exit();
}

else if(!filter_var($email,FILTER_VALIDATE_EMAIL)&& !preg_match("/^[a-zA-Z0-9]*$/",$uname)){
header("Location: ../includes/signup.php?error=invalidename&duname");
exit();
}
else if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
header("Location:../includes/signup.php?error=invalidemail&uname");
exit();
}
else if(!preg_match("/^[a-zA-Z0-9]*$/",$uname)){
header("Location: ../includes/signup.php?error=invaliduname&ename");
exit();
}
else if($password!==$conformpassword){
header("Location: ../includes/signup.php?error=passwordcheck&funame");
exit();
}
   else{
      ...
      if(!mysqli_stmt_prepare($stmt,$sql)){
       header("Location: ../includes/signup.php?error=sqlerror");
       exit();
        }
      else {
   ...
   if($checkresult >0){
    header("Location: ../includes/signup.php?error=usertaken&ename");
    exit();
    }
   else
   {
